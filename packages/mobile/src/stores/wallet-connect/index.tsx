...
export class WalletConnectStore {
  ...
  constructor(
    ...
    protected readonly permissionManagerStore: PermissionService
  ) {
    makeObservable(this);
  ...
  protected async init(): Promise<void> {
    const projectId =
      process.env["WC_PROJECT_ID"] || "8d611785b5b4298436793509ca6198df";
    if (!projectId || true) {
      return;
    }
  ...
  async pair(uri: string, fromDeepLink?: boolean) {
    ...
    const signClient = await this.ensureInit();
    console.log("pair:2", signClient.logger);

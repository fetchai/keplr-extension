...
export class CosmJSFetchOfflineSignerOnlyAmino implements OfflineSigner {
  constructor(
    protected readonly chainId: string,
  ...
  async getAccounts(): Promise<AccountData[]> {
    const key = await this.signingApi.getKey(this.chainId);
  ...
  async signAmino(
    ...
    const key = await this.signingApi.getKey(signDoc.chain_id);
...
export class CosmJSFetchOfflineSigner
  ...
  async signDirect(
    ...
    const key = await this.signingApi.getKey(signDoc.chainId);
